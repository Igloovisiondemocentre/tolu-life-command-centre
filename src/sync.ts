...
export type SyncModel={missions:Mission[];timeline:TimelineEntry[];calendar:string[][];financialHealth:string[][];financeDebts:string[][];contacts:string[][];usefulEmails:UsefulEmail[];importLog:ImportRecord[];syncedAt:string;sourceName:string};
...
export async function parseCommandCentre(file:File):Promise<SyncModel>{
  ...
  return {missions,timeline,calendar:sheets.get("Calendar & Deadlines")||[],financialHealth:sheets.get("Financial Health")||[],financeDebts:sheets.get("Finance & Debts")||[],contacts:sheets.get("Contacts & Threads")||[],usefulEmails,importLog,syncedAt:new Date().toISOString(),sourceName:file.name};
}
