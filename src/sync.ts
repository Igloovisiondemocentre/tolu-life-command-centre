...
export type UsefulEmail={date:string;category:string;sender:string;subject:string;summary:string;relevance:string;value:string;deadline:string;action:string;status:string;taskId:string;sourceUrl:string};
export type ImportRecord={id:string;name:string;sourceType:string;modified:string;processed:string;status:string;result:string;sourceUrl:string};
export type WhatsAppChat={id:string;name:string;type:string;participants:string;scopeRule:string;first:string;last:string;window:string;messages:number;fileId:string;modified:string;lastProcessed:string;status:string;taskIds:string[];summary:string;sourceUrl:string;notes:string};
export type ControlRequest={requestId:string;target:string;action:string;field:string;details:string;submittedBy:string;requestedOn:string;ready:string;status:string;processedOn:string;result:string};
export type SyncModel={missions:Mission[];timeline:TimelineEntry[];calendar:string[][];financialHealth:string[][];financeDebts:string[][];contacts:string[][];usefulEmails:UsefulEmail[];importLog:ImportRecord[];whatsAppChats:WhatsAppChat[];taskControls:ControlRequest[];syncedAt:string;sourceName:string};
...
export async function parseCommandCentre(file:File):Promise<SyncModel>{
  ...
  const usefulEmails=objects(sheets.get("Useful Emails")||[]).filter(r=>r["Subject / Opportunity"]).map(r=>({date:Number(r["Email Date"])?excelDate(Number(r["Email Date"])):r["Email Date"],category:r.Category,sender:r["Sender / Organisation"],subject:r["Subject / Opportunity"],summary:r.Summary,relevance:r.Relevance,value:r["Value / Pay / Benefit"],deadline:r["Deadline / Expiry"],action:r["Recommended Action"],status:r.Status,taskId:r["Related Task ID"],sourceUrl:r["Source URL"]||r["OPEN GMAIL"]}));
  const importLog=objects(sheets.get("Import Log")||[]).filter(r=>r["File ID"]).map(r=>({id:r["File ID"],name:r["File Name"],sourceType:r["Source Type"],modified:r["Source Modified"],processed:r["Processed At"],status:r.Status,result:r.Result,sourceUrl:r["Source File"]}));
  const whatsAppChats=objects(sheets.get("WhatsApp Chats")||[]).filter(r=>r["Chat ID"]).map(r=>({id:r["Chat ID"],name:r["Chat Name"],type:r["Chat Type"],participants:r.Participants,scopeRule:r["Scope Rule"],first:r["Export First Message"],last:r["Export Last Message"],window:r["Six-Month Window"],messages:Number(r["Messages Reviewed"]||0),fileId:r["Current File ID"],modified:r["File Modified"],lastProcessed:r["Last Processed Message"],status:r["Processing Status"],taskIds:(r["Related Task IDs"]||"").split(",").map(x=>x.trim()).filter(Boolean),summary:r["Actionable Summary"],sourceUrl:r["Source URL"]||r["OPEN EXPORT"],notes:r.Notes}));
  const taskControls=objects((sheets.get("Task Controls")||[]).slice(4)).filter(r=>r["Task ID or Title"]).map(r=>({requestId:r["Request ID"],target:r["Task ID or Title"],action:r.Action,field:r.Field,details:r["New Value / Details"],submittedBy:r["Submitted By"],requestedOn:r["Requested On"],ready:r["Ready?"],status:r["Processing Status"],processedOn:r["Processed On"],result:r["Result / Notes"]}));
  return {missions,timeline,calendar:sheets.get("Calendar & Deadlines")||[],financialHealth:sheets.get("Financial Health")||[],financeDebts:sheets.get("Finance & Debts")||[],contacts:sheets.get("Contacts & Threads")||[],usefulEmails,importLog,whatsAppChats,taskControls,syncedAt:new Date().toISOString(),sourceName:file.name};
}
